...
// ─── PATCH /api/bookings/:id/admin-checkin  (admin only — manual check-in) ──
router.patch('/:id/admin-checkin', auth, (req, res) => {
  if (req.user.role !== 'admin') return res.status(403).json({ error: 'Admin only' });
  const b = db.prepare(`
    SELECT b.*, u.name as user_name, u.email as user_email, r.name as room_name, r.floor as room_floor, r.color as room_color
    FROM bookings b JOIN users u ON u.id=b.user_id JOIN rooms r ON r.id=b.room_id
    WHERE b.id = ?
  `).get(req.params.id);
  if (!b) return res.status(404).json({ error: 'Booking not found' });
  if (b.status !== 'approved') return res.status(400).json({ error: 'Booking must be approved before check-in.' });
  if (b.checkin_at) return res.status(400).json({ error: 'Already checked in at ' + b.checkin_at });
  db.prepare("UPDATE bookings SET checkin_at = datetime('now') WHERE id = ?").run(req.params.id);
